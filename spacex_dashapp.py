...
def get_success_chart(entered_site, slider1):
    min1, max1 = slider1
    df_new = spacex_df[(spacex_df["Payload Mass (kg)"] > min1) & (spacex_df['Payload Mass (kg)'] <max1)]
    if entered_site == 'ALL':
        df = spacex_df
        fig = px.scatter(df_new, x = 'Payload Mass (kg)', y = 'class', color = "Booster Version Category", 
        title = "")
        return fig
    else:
